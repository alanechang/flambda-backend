...
let unsafe_fill : ('a : word) array -> int -> int -> ('a : word) -> unit =
  fun a ofs len v ->
    for i = ofs to ofs + len - 1 do unsafe_set a i v done

let make : int -> ('a : word) -> ('a : word) array = fun n v ->
...
let unsafe_sub: ('a : word) array -> int -> int -> ('a : word) array = fun a ofs len ->
  let result = unsafe_create len in
  unsafe_blit a ofs result 0 len;
  result

let append_prim: ('a : word) array -> ('a : word) array -> ('a : word) array = fun a1 a2 ->
  let l1 = length a1 in
  let l2 = length a2 in
...
let ensure_ge (x:int) y =
  if x >= y then x else invalid_arg "Nativeint_u_array.concat"

let rec sum_lengths acc = function
  | [] -> acc
  | hd :: tl -> sum_lengths (ensure_ge (length hd + acc) acc) tl

let concat: ('a : word) array list -> ('a : word) array = fun l ->
...
let init l f =
  ...
  if l < 0 then invalid_arg "Nativeint_u_array.init"
...
let sub a ofs len =
  ...
  then invalid_arg "Nativeint_u_array.sub"
  else unsafe_sub a ofs len

let fill a ofs len v =
  if ofs < 0 || len < 0 || ofs > length a - len
  then invalid_arg "Nativeint_u_array.fill"
  else unsafe_fill a ofs len v

let blit a1 ofs1 a2 ofs2 len =
  if len < 0 || ofs1 < 0 || ofs1 > length a1 - len
             || ofs2 < 0 || ofs2 > length a2 - len
  then invalid_arg "Nativeint_u_array.blit"
...
let iter2 f a b =
  if length a <> length b then
    invalid_arg "Nativeint_u_array.iter2: arrays must have the same length"
  else
...
let map2 f a b =
  ...
  let lb = length b in
  if la <> lb then
    invalid_arg "Nativeint_u_array.map2: arrays must have the same length"
  else begin
 ...
 *   tolist (length a - 1) []
 *
 * (* Cannot use List.length here because the List module depends on Nativeint_u_array. *)
...
let for_all2 p l1 l2 =
  let n1 = length l1
  and n2 = length l2 in
  if n1 <> n2 then invalid_arg "Nativeint_u_array.for_all2"
  else let rec loop i =
    if i = n1 then true
...
let exists2 p l1 l2 =
  let n1 = length l1
  and n2 = length l2 in
  if n1 <> n2 then invalid_arg "Nativeint_u_array.exists2"
  else let rec loop i =
    if i = n1 then false
...
let[@inline always] compare x y = Nativeint.compare (to_boxed x) (to_boxed y)
let mem x a =
  let n = length a in
  let rec loop i =
    if i = n then false
 ...
 *   let na = length a in
 *   let nb = length b in
 *   if na <> nb then invalid_arg "Nativeint_u_array.combine";
 *   if na = 0 then (empty ())
 *   else begin
