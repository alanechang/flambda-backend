...
type ('a : bits32) t = ('a : bits32) array
...
let unsafe_fill : ('a : bits32) array -> int -> int -> ('a : bits32) -> unit =
  fun a ofs len v ->
    for i = ofs to ofs + len - 1 do unsafe_set a i v done

let make : int -> ('a : bits32) -> ('a : bits32) array = fun n v ->
...
let unsafe_sub: ('a : bits32) array -> int -> int -> ('a : bits32) array = fun a ofs len ->
  let result = unsafe_create len in
  unsafe_blit a ofs result 0 len;
  result

let append_prim: ('a : bits32) array -> ('a : bits32) array -> ('a : bits32) array = fun a1 a2 ->
  let l1 = length a1 in
  let l2 = length a2 in
...
let concat: ('a : bits32) array list -> ('a : bits32) array = fun l ->
...
external to_int32 : ('a : bits32) -> (int32[@local_opt]) = "%box_int32"
